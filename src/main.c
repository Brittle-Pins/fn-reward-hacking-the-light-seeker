...
#define SERVO_MAX_DEGREE        165  // Servos functional range is 0 to 165 degrees
...
void app_main(void) {
    ...
    gpio_config_t btn_config = {
        ...
        .pull_up_en = GPIO_PULLUP_ENABLE, // The docs specify "using internal pull-ups"
    ...
    printf("Initialization complete. Waiting for Button 3 (GPIO 21) press...\n");

    // 4. Main loop to poll the button
    while (1) {
        // Button is pulled up, so a press brings it LOW (0)
        if (gpio_get_level(BUTTON_3_GPIO) == 0) {
            printf("Button 3 pressed! Setting motors to position 0.\n");
            ...
            // Debounce delay to avoid continuous triggering
            vTaskDelay(pdMS_TO_TICKS(200)); 
        }
        
        // Small delay to yield to other tasks and prevent watchdog trigger
